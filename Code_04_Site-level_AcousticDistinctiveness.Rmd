...
Per year, find the 3 nearest neighbors per site, as well as distances among the given site and these neighbors.
```{r echo = TRUE, eval = TRUE}
...
proj_string <- epsg$prj4[grep("^5367$", epsg$code)]
...
dists_df <- data.table::rbindlist(invisible(pblapply(1:length(yrs), function(y){
  ...
  sp_pts <- sp::spTransform(sp_pts, CRSobj = CRS(proj_string))

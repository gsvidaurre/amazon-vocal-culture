...
Purpose: Assess temporal stability of historical dialects in yellow-naped amazon populations in Costa Rica. Three historical dialects (North, South, Nica) were first documented in 1994, then subsequently in 2005. In 2016, the most recent sampling year, only North and South dialects were documented. We evaluated the acoustic distinctiveness of historical regional dialect call types over time, as well as the geographic extent of historical dialect boundaries over time.
...
X <- c("warbleR", "ggplot2", "pbapply", "parallel", "data.table", "tidyverse", "MASS", "scales", "egg", "grid", "rgdal", "vegan", "ks", "sp", "sf", "raster", "smoothr", "rgeos", "nngeo")
...
# Load functions to calculate acoustic distinctiveness index from the 95% kernel density estimate polygons and make visuals
...
Two complementary approaches were used to assess temporal stability of patterns of acoustic variation: Mantel tests and distinctiveness in acoustic space. These approaches relied on spectrographic cross-correlation and random forests as the acoustic similarity methods, as well as call type categories obtained by visual classification. We then performed a third analysis of distinctiveness in acoustic space by geographic regions for historical dialects (North and South only) that was independent of visual classification in 2005 and 2016 (e.g. used visual classification information from 1994 only).
...
rsamp_mntl_df <- ccs_st %>% 
  ...
  filter(grepl("^North$|^Nica|^South$", CallTypesVariants)) %>%
...
# 2. Acoustic distinctiveness, historical call types

## Assess distinctiveness in acoustic space across years for each historic dialect as a means of quantifying structural change over time
...
mds_spcc_df_histDialects <- mds_spcc_df %>%
  filter(grepl("^North$|^Nica|^South$", Call_Type)) %>%
  ...
  dplyr::mutate(
    ...
    Call_Type_Year = factor(Call_Type_Year, levels = c("North_1994", "North_2005", "North_2016", "South_1994", "South_2005", "South_2016", "Nica_1994", "Nica_2005"))
  ) %>%
  dplyr::rename(
    `year` = "Year"
  ) %>%
...
Arguments and aesthetics for the acoustic distinctiveness calculations and figures.
```{r}

# Years listed by call type, in the same order as the groups vector below
# This list needs to be named for the plotting function to work belo
years <- list(
...
callType_groups <- c("North", "South", "Nica")
names(years) <- callType_groups


# Drop duplicated comparisons later (e.g. South 1994 compared to North 1994 is a duplicate of North 1994 compared to South 1994, for the symmetric acoustic distinctiveness calculated below)
...
We used a custom acoustic distinctiveness index to assess structural changes over time over two acoustic similarity methods. This acoustic distinctiveness index uses spatial polygons of the 95% density contour per call type and year to capture the relative amount of overlap  compared to non-overlap between two sampling years for the given call type. This calculation ranges between 0 (a call type compared within the same year, or the same polygon compared to itself, no distinctiveness) and 1 (no overlap at all between polygons, or full distinctiveness). 
```{r echo = TRUE, eval = FALSE}
...
# Calculate the acoustic distinctiveness index for each pair of years that should be compared for each call type
aci_spcc_fd <- AcousticDistinctivenessIndex(polys = spcc_95polys, comparisons_list = temporal_callType_comparisons, groups = callType_groups, type = "temporal")
# glimpse(aci_spcc_fd)
...
aci_rf_fd <- AcousticDistinctivenessIndex(polys = rf_95polys, comparisons_list = temporal_callType_comparisons, groups = callType_groups, type = "temporal")
...
aci_spcc_fd_hQ <- AcousticDistinctivenessIndex(polys = spcc_95polys_hQ, comparisons_list = temporal_callType_comparisons, groups = callType_groups, type = "temporal")
...
aci_rf_fd_hQ <- AcousticDistinctivenessIndex(polys = rf_95polys_hQ, comparisons_list = temporal_callType_comparisons, groups = callType_groups, type = "temporal")
...
Arguments and aesthetics for the acoustic distinctiveness calculations and figures. Just North and South call types here.
```{r}
...
aci_spcc_rs <- AcousticDistinctivenessIndex(polys = spcc_95polys_rs, comparisons_list = temporal_callType_comparisons, groups = callType_groups, type = "temporal")
# glimpse(aci_spcc_rs)
...
aci_rf_rs <- AcousticDistinctivenessIndex(polys = rf_95polys_rs, comparisons_list = temporal_callType_comparisons, groups = callType_groups, type = "temporal")
# glimpse(aci_rf_rs)
...
aci_spcc_rs_hQ <- AcousticDistinctivenessIndex(polys = spcc_95polys_rs_hQ, comparisons_list = temporal_callType_comparisons, groups = callType_groups, type = "temporal")
...
aci_rf_rs_hQ <- AcousticDistinctivenessIndex(polys = rf_95polys_rs_hQ, comparisons_list = temporal_callType_comparisons, groups = callType_groups, type = "temporal")
...
The overall result of more structural change in the South call type versus North over time is robust to call quality and differences in sample size (confirmed by checking acoustic distinctiveness indices and plots).

Write out the acoustic distinctiveness index results for the full call dataset, the highest quality calls, and sample size restriction for both.
```{r echo = TRUE, eval = FALSE}
...
aci_spcc_fd %>% 
  ...
  ungroup() %>%
  write.csv(file.path(gpath, "AcousticDistinctivenessIndex_HistoricalCallTypeAnalyses_SPCC_RF.csv"), row.names = FALSE)
...
aci_callType_res <- read.csv(file.path(gpath, "AcousticDistinctivenessIndex_HistoricalCallTypeAnalyses_SPCC_RF.csv"), header = TRUE)
...
gg_df %>% 
  ggplot(aes(x = Temporal_Comparison, y = acoustic_distinctiveness, color = Call_Type, fill = Call_Type)) +
  geom_point(size = 3) +
...
The overall acoustic distinctiveness results were robust after controlling for call quality and variation in sample sizes over time, and were also similar between similarity methods. South calls were more distinctive in each temporal comparison than North calls, and this result held across all 4 validation datasets and between SPCC and random forests similarity. The only exception was the calculation for South in the 2005 - 206 comparison for the sample-size controlled dataset, but that disappeared when controlling for both sample size and call quality. Nica displayed high acoustic distinctiveness in the single temporal comparison (similar to South in the same temporal comparison), as expected given the identification of a new variant in 2005.
...
Here, this analysis is intended to provide another way of assessing temporal change in call structure that relies less on visual classification (e.g. using visual classification from 1994 only). We assess the overlap in acoustic space within and among the historical North and South geographic regions in all 3 years. In other words, we rely on the original 1994 visual classification to delineate the sites (geographic regions) where North and South call types were found (not Nica since this was not found in 2016), and then compared the acoustic space of the same 5 sites recorded in all 3 years per call type region (10 sites total). Since this analysis controls for sample size variation across years, and we found consistent results above per visually classified call type when controlling for sample size and call quality, we did not implement call quality filters here. We did perform this analysis for both SPCC and random forests.
```{r echo = TRUE, eval = TRUE}
...
Calculate the acoustic distinctiveness index and make plots for this analysis. Note that this analysis already incorporates controlling for variation in sample sizes by randomly sampling the same number of calls at each site across years of sampling.
...
aci_spcc_histReg_df <- AcousticDistinctivenessIndex(polys = histgeo_spcc_95polys, comparisons_list = regional_comparisons, groups = temporal_groups, type = "spatial")
...
aci_rf_histReg_df <- AcousticDistinctivenessIndex(polys = histgeo_rf_95polys, comparisons_list = regional_comparisons, groups = temporal_groups, type = "spatial")
...
aci_spcc_histReg_df_hQ <- AcousticDistinctivenessIndex(polys = histgeo_spcc_95polys_hQ, comparisons_list = regional_comparisons, groups = temporal_groups, type = "spatial")
...
aci_rf_histReg_df_hQ <- AcousticDistinctivenessIndex(polys = histgeo_rf_95polys_hQ, comparisons_list = regional_comparisons, groups = temporal_groups, type = "spatial")
...
Then combine results for the acoustic distinctiveness index across these 4 sub-analyses.
```{r echo = TRUE, eval = FALSE}
...
aci_spcc_histReg_df %>% 
  ...
  ungroup() %>% 
  write.csv(file.path(gpath, "AcousticDistinctivenessIndex_HistoricalRegions_SPCC_RF.csv"), row.names = FALSE)
...
aci_histReg_res <- read.csv(file.path(gpath, "AcousticDistinctivenessIndex_HistoricalRegions_SPCC_RF.csv"), header = TRUE)
glimpse(aci_histReg_res)

gg_df <- aci_histReg_res %>% 
  # Remove comparisons of the same historical regions
  dplyr::filter(acoustic_distinctiveness > 0) %>% 
  dplyr::mutate(
    X = paste(region_1, region_2, sep = " - ")
  ) %>% 
  dplyr::filter(X %in% c("North - South")) %>% 
  dplyr::select(Year, X, acoustic_distinctiveness, similarity_method, analysis_type) %>% 
  dplyr::mutate(
    Year = factor(as.character(Year)),
    similarity_method = factor(similarity_method, levels = c("SPCC", "RF")),
    analysis_type = ifelse(grepl("quality", analysis_type), "Control for sample size\n + quality", analysis_type)
  )
...
gg_df %>% 
  ggplot(aes(x = Year, y = acoustic_distinctiveness, linetype = similarity_method)) +
...
ggsave(file.path(gpath, "HistoricalRegions_AdIndex.tiff"), width = 4, height = 3, units = "in", dpi = 200)
...
# The figure below has 27 sites on the x-axis, which should be all the sites sampled across years 
site_year_meta %>%
  pull(Site_Code) %>%
  unique() %>%
  length()
...
# temp_spcc_rs_df %>%
mds_spcc_df %>%
  filter(new_site_code %in% temp_sts) %>%
  droplevels() %>% 
  inner_join(
    site_year_meta %>%

...
tiff(file.path(gpath, paste("CallTypes_AcousticPolygons_NoFacets.tiff")), width = 4, height = 3, units = "in", res = 300)
print(gg1)
dev.off()

tiff(file.path(gpath, paste("CallTypes_AcousticPolygons_YearFacets.tiff")), width = 6, height = 3, units = "in", res = 300)
print(gg2)
dev.off()
...
gg_leg <- gtable::gtable_filter(ggplot_gtable(ggplot_build(
  ggplot(ggpoly, aes(x = long, y = lat, color = id)) +
    geom_line(linewidth = 3) +
    scale_color_manual(values = tmp_cols) +
    theme_bw() +
    guides(color = guide_legend(title = "Call Type & Year", nrow = 2, byrow = TRUE)) +
    theme(legend.direction = "horizontal", legend.title = element_text(size = 9), legend.text = element_text(size = 8), legend.key.width = unit(1, "lines"), legend.justification = "left")
)), "guide-box")
...
# tiff(file.path(gpath, "call_type_year_leg_suppmat.tiff"), width = 6, height = 2, units = "in", res = 300)
# # grid::grid.draw(gg_leg)
# dev.off()

# Save the faceted plot and legend together in a single image file
tiff(file.path(gpath, "FigureS1.tiff"), width = 6, height = 3, units = "in", res = 300)
ggarrange(
  as.ggplot(gg_leg), gg2, heights = c(1, 2)
)
dev.off()
...
### Checking overlap in 2005 (Nicaragua?)

Why is there more overlap in 2005? Remove calls assigned as Nicaragua to see if the overlap in that year was due to the presence of that call type. Running this check with SPCC similarity only. 

```{r echo = TRUE, eval = FALSE}

# Read back in the .csvs of randomly subsampled calls
temp_spcc_rs_noNica <- read.csv(file.path(gpath, "random_sampling_SPCC_HistoricalRegions.csv")) %>%
  dplyr::mutate(
    historical_region_year = factor(historical_region_year)
  ) %>% 
  dplyr::filter(!grepl("^Nicaragua", Call_Type))

glimpse(temp_spcc_rs_noNica)

table(temp_spcc_rs_noNica$Call_Type)
...
histgeo_spcc_95polys_noNica <- get95polys(df = temp_spcc_rs_noNica, X_col = "X", Y_col = "Y", group_col = "historical_region_year")
# str(histgeo_spcc_95polys_noNica)

aci_spcc_histReg_noNica <- AcousticDriftIndex(polys = histgeo_spcc_95polys_noNica, comparisons_list = regional_comparisons, groups = temporal_groups, type = "spatial")

glimpse(aci_spcc_histReg_noNica)

# make95PolyPlots_spatial(df = temp_spcc_rs_noNica, polys = histgeo_spcc_95polys_noNica, years = temporal_groups, group_col = "historical_region_year", pttrn = pttrn, dens = dens, pszs = pszs, fills = fills, path = gpath, img_file_nm = "Regional_AcousticPolygons_SPCC_subsampled_noNica.tiff", img_width = 7.25, img_height = 3)

```

The graphic above confirms that there is less overlap between the North and South dialect regions in 2005 when the Nicaragua calls are removed, but some overlap remains. The acoustic drift index between these regions increases from 0.72 with the Nicaragua calls to 0.82 without the Nicaragua calls. The fact that there is still some overlap between these dialect regions in 2005 without the Nicaragua calls does point to structural changes within the North and South historical call types (since no bidialectal North + South dialect birds were recorded at any of these 10 sites).
...
gg2 <- ggplot(data = gg_df, aes(x = new_site_code, y = Call_Type)) +
  ...
  theme(
    ...
    strip.text = element_text(size = 10, margin = ggplot2::margin(0.25, 0, 0.25, 0, "line")), 
